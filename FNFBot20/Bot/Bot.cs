...
namespace FNFBot20
{
    public class Bot
    {
        ...
        public void HandleNote(FNFSong.FNFNote n)
        {
            ...
            int holdDuration = Convert.ToInt32(n.Length);
            
            if (shouldHold)
            {
                Form1.WriteToConsole($"Hold note: Type={n.Type}, Length={holdDuration}ms");
            }
            ...
            switch (n.Type)
            {
                ...
                case FNFSong.NoteType.RLeft:
                    if (shouldHold)
                    {
                        kBot.KeyDown(0x41, 0x1e);
                        new Thread(() =>
                        {
                            Thread.Sleep(holdDuration);
                            kBot.KeyUp(0x41, 0x1e);
                        }).Start();
                    }
                    else
                    {
                        kBot.KeyPress(0x41, 0x1e);
                    }
                ...
                case FNFSong.NoteType.RDown:
                    if (shouldHold)
                    {
                        kBot.KeyDown(0x53, 0x1f);
                        new Thread(() =>
                        {
                            Thread.Sleep(holdDuration);
                            kBot.KeyUp(0x53, 0x1f);
                        }).Start();
                    }
                    else
                        kBot.KeyPress(0x53, 0x1f);
                ...
                case FNFSong.NoteType.RUp:
                    if (shouldHold)
                    {
                        kBot.KeyDown(0x57, 0x11);
                        new Thread(() =>
                        {
                            Thread.Sleep(holdDuration);
                            kBot.KeyUp(0x57, 0x11);
                        }).Start();
                    }
                    else
                        kBot.KeyPress(0x57, 0x11);
                ...
                case FNFSong.NoteType.RRight:
                    if (shouldHold)
                    {
                        kBot.KeyDown(0x44, 0x20);
                        new Thread(() =>
                        {
                            Thread.Sleep(holdDuration);
                            kBot.KeyUp(0x44, 0x20);
                        }).Start();
                    }
                    else
                        kBot.KeyPress(0x44, 0x20);
